/**
 * arrayToTree 将给定的数组转换为树形结构。
 * @param arr - 原始数组，其中每个元素包含id和pid属性，pid表示父级id。
 * @returns 返回转换后的树形结构数组。
 */

export function arrayToTree(arr: any[]) {
  // 初始化
  const res: any = []

  // 使用Map存储数组元素，以id为键，元素本身为值
  const map = new Map()

  // 遍历原始数组，将每个元素以id为键，其余属性为值的对象存入map
  arr.forEach(item => map.set(item.id, item))

  // 再次遍历原始数组，根据pid将元素组织成树形结构
  arr.forEach(item => {
    // 获取当前元素的父级元素
    const parent = item.pid && map.get(item.pid)
    // 如果存在父级元素，则将当前元素添加到父级元素的children数组中
    if (parent) {
      if (parent?.children) {
        parent.children.push(item)
      } else {
        // 如果父级元素没有children属性，则创建子元素数组，并将当前元素作为第一个元素
        parent.children = [item]
      }
    } else {
      res.push(item)
    }
  })

  return res
}
